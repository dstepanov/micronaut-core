...
public class AnnotationMetadataWriter extends AbstractClassFileWriter {
    ...
    static {
        ...
        if (mapListSupported) {
            MAP_OF = new Method[10];
            LIST_OF = new Method[10];
...
//                staticInit.visitLabel(new Label());
                ...
                if (!typeOnly) {
                    pushMapOf(staticInit, annotationValues, v -> pushValue(owningType, classWriter, staticInit, v, loadTypeMethods));
...
//                staticInit.visitLabel(falseCondition);
    ...
    private static <T> void pushMapOf(GeneratorAdapter generatorAdapter, Map<? extends CharSequence, T> annotationData,
        ...
        if (MAP_OF != null) {
            if (entrySet.size() <= MAP_OF.length) {
                ...
                // start a new array
                pushNewArray(generatorAdapter, Map.Entry.class, entrySet.size() * 2);
                int i = 0;
                ...
                while (iterator.hasNext()) {
                    ...
                    pushValue.accept(entry.getValue());
                    // entry
                    generatorAdapter.invokeStatic(Type.getType(Map.class), MAP_ENTRY);
    ...
    private static void pushCreateAnnotationData(
        ...
        pushMapOf(methodVisitor, annotationData, attributes -> {
            pushMapOf(methodVisitor, attributes, v -> pushValue(declaringType, declaringClassWriter, methodVisitor, v, loadTypeMethods));
        });
    }
